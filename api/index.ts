...
const admin = SB_SERVICE ? createClient(SB_URL, SB_SERVICE) : supabase;
...
const FALLBACKS = ['nvidia/nemotron-4-340b-instruct:free','google/gemini-2.0-flash-exp:free','deepseek/deepseek-r1:free','meta-llama/llama-3.1-70b-instruct:free','meta-llama/llama-3.1-8b-instruct:free','mistralai/mistral-7b-instruct:free','qwen/qwen-2-7b-instruct:free'];

function resolveModel(m) { return MODELS[m] || (m?.includes('/') ? m : DEF_MODEL); }
...
async function runAI(prompt, sysPrompt, model, temp, max, key) {
  if (MOCK || !key) {
    await new Promise(r => setTimeout(r, 400));
    return { text: `🤖 [Mock] "${prompt.slice(0,60)}..." | ${model} | temp=${temp} | Set OPENROUTER_API_KEY for real AI`, tokens: 50, model };
  }
  const msgs = [];
  ...
  try {
    ...
    if (path === '/api/models') {
      return res.json({ models: [
        { id: 'nvidia/nemotron-4-340b-instruct:free', name: 'Nemotron 4 340B', ctx: 32768 },
        { id: 'google/gemini-2.0-flash-exp:free', name: 'Gemini 2.0 Flash', ctx: 1048576 },
        { id: 'deepseek/deepseek-r1:free', name: 'DeepSeek R1', ctx: 65536 },
        { id: 'meta-llama/llama-3.1-70b-instruct:free', name: 'Llama 3.1 70B', ctx: 32768 },
        { id: 'meta-llama/llama-3.1-8b-instruct:free', name: 'Llama 3.1 8B', ctx: 32768 },
        { id: 'mistralai/mistral-7b-instruct:free', name: 'Mistral 7B', ctx: 32768 },
        { id: 'qwen/qwen-2-7b-instruct:free', name: 'Qwen 2 7B', ctx: 32768 },
        { id: 'openrouter/owl-alpha:free', name: 'OWL Alpha', ctx: 8192 },
      ]});
    }
    ...
    if (path === '/api/run' && req.method === 'POST') {
      ...
      const { agentId, systemPrompt, userMessage, modelEngine, temperature, maxTokens, toolsConfig } = req.body || {};
      if (!userMessage) return res.status(400).json({ error: 'userMessage required' });

      let apiKey = OR_KEY;
      const { data: prof } = await supabase.from('profiles').select('openrouter_api_key').eq('id', userId).single();
      if (prof?.openrouter_api_key) apiKey = prof.openrouter_api_key;
      ...
      const result = await runAI(
        ...
        modelEngine || agentCfg.model_engine || DEF_MODEL,
        temperature ?? agentCfg.temperature ?? 0.7,
        maxTokens ?? agentCfg.max_tokens ?? 2048,
        apiKey
      );
      const ms = Date.now() - t0;
      try { await supabase.from('telemetry_logs').insert({ agent_id: agentId, user_id: userId, tokens_used: result.tokens, latency_ms: ms, status: 'success', message: `[OK] ${result.model} ${ms}ms` }); } catch {}
    ...
    if (path === '/api/agents' && req.method === 'POST') {
      const userId = (await uid(req)) || (ANON ? '00000000-0000-0000-0000-000000000000' : null);
      if (!userId) return res.status(401).json({ error: 'Auth required' });
      const { name, description, system_prompt, model_engine, temperature, max_tokens, tools_config } = req.body || {};
      if (!name) return res.status(400).json({ error: 'name required' });
      const { data, error } = await supabase.from('agents').insert({ user_id: userId, name, description: description || '', system_prompt: system_prompt || '', model_engine: model_engine || DEF_MODEL, temperature: temperature ?? 0.7, max_tokens: max_tokens ?? 2048, tools_config: tools_config || {} }).select().single();
      if (error) return res.status(500).json({ error: error.message });
